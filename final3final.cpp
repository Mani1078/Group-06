...
public:
    ...
    void insertRecord(string tableName, Args... args) {
        ...
        (recordData.emplace_back(to_string_convert(args)), ...); // تبدیل تمام args به string
    
        records.emplace_back(tableName, recordData);
        cout << "Record added to " << tableName << "!\n";
    }
    
    // تابع کمکی برای تبدیل به رشته
    template<typename T>
    string to_string_convert(const T& value) {
...
int main() {
   ...
   // myDatabase.addTable(users);
    ...
    myDatabase.insertRecord("Users", 2, "amir","lanat be in vaziat !!!!!","maskhare bazi!","bia!","ta sobh arguman bezar in to !","man ravanni am");
